...
def test_token_entity_intersection():
    # included
    intsec = determine_intersection(
        CH_correct_segmentation[1], CH_correct_entity
    )
    assert intsec == len(CH_correct_segmentation[1].text)

    # completely outside
    intsec = determine_intersection(
        CH_correct_segmentation[2], CH_correct_entity
    )
    assert intsec == 0
...
def test_token_entity_boundaries():
    ...
    assert not does_token_cross_borders(
        CH_wrong_segmentation[1], CH_correct_entity
    )
    ...
    # completely outside
    assert not is_token_within_entity(
        CH_correct_segmentation[0], CH_correct_entity
    )
    assert not does_token_cross_borders(
        CH_correct_segmentation[0], CH_correct_entity
    )
    ...
    # border crossing
    assert not is_token_within_entity(
        CH_wrong_segmentation[0], CH_correct_entity
    )
...
def test_label_merging():
    ...
    assert all(
        merge_labels(aligned_predictions, "A") == ["O", "O", "O", "O", "O"]
    )
...
def test_run_cv_evaluation():
    ...
    assert (
        len(entity_results.train["CRFEntityExtractor"]["Accuracy"]) == n_folds
    )
    assert (
        len(entity_results.train["CRFEntityExtractor"]["Precision"]) == n_folds
    )
    assert (
        len(entity_results.train["CRFEntityExtractor"]["F1-score"]) == n_folds
    )
    assert len(entity_results.test["CRFEntityExtractor"]["Accuracy"]) == n_folds
    assert (
        len(entity_results.test["CRFEntityExtractor"]["Precision"]) == n_folds
    )
...
def test_evaluate_entities_cv_empty_tokens():
    ...
    result = align_entity_predictions(
        EN_targets, EN_predicted, [], mock_extractors
    )
