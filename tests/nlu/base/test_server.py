...
def test_get_parse(app, response_test):
    ...
    assert all(
        prop in rjs
        for prop in ["project", "entities", "intent", "text", "model"]
    )
...
def test_evaluate(app, rasa_default_train_data):
    response = app.post(
        "http://dummy-uri/evaluate", json=rasa_default_train_data
    )
    time.sleep(3)
    app.flush()
    response = yield response
...
def test_unload_model_error(app):
    ...
    assert rjs["error"] == (
        "Failed to unload model my_model for project " "default."
    )
