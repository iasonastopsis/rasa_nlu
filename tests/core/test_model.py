...
def test_get_latest_model(trained_model):
    ...
    path_of_latest = os.path.join(
        os.path.dirname(trained_model), "latest.tar.gz"
    )
...
def _fingerprint(
    config=None, domain=None, rasa_version="1.0", stories=None, nlu=None
):
    return {
...
def _project_files(
    ...
    return {
        k: v if v is None else os.path.join(project, v)
...
def test_create_fingerprint_from_invalid_paths(project, project_files):
    ...
    expected = _fingerprint(
        [], [], rasa_version=rasa.__version__, stories=[], nlu=[]
    )
...
def test_rasa_packaging(trained_model, project, use_fingerprint):
    ...
    assert (
        os.path.exists(os.path.join(unpacked, FINGERPRINT_FILE_PATH))
        == use_fingerprint
    )
    assert os.path.exists(os.path.join(unpacked, "core"))
    assert os.path.exists(os.path.join(unpacked, "nlu"))
