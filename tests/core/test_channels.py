...
def test_handling_of_telegram_user_id():
    ...
    rasa.core.channels.channel.register(
        [input_channel], app, route="/webhooks/"
    )
...
async def test_callback_calls_endpoint():
    ...
    with aioresponses() as mocked:
        ...
        await output.send_response(
            "test-id",
            {"text": "Hi there!", "image": "https://example.com/image.jpg"},
        )
...
def test_slack_message_sanitization():
    ...
    assert (
        len(
            [
                sanitized
                for sanitized, target in zip(
                    sanitized_messages, target_messages
                )
                if sanitized != target
            ]
        )
...
def test_channel_inheritance():
    with mock.patch.object(sanic.Sanic, "run", fake_sanic_run):
        ...
        ).startswith("/webhooks/rasa")
        assert routes_list.get(
            "custom_webhook_RasaChatInput.receive"
        ).startswith("/webhooks/rasa/webhook")
