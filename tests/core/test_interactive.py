...
async def test_print_history(mock_endpoint):
    ...
    with aioresponses() as mocked:
        mocked.get(
            url, body=tracker_dump, headers={"Accept": "application/json"}
        )
...
async def test_is_listening_for_messages(mock_endpoint):
    ...
    with aioresponses() as mocked:
        mocked.get(
            url, body=tracker_dump, headers={"Content-Type": "application/json"}
        )
...
def test_as_md_message():
    parse_data = {
        ...
        "entities": [
            {"start": 12, "end": 16, "entity": "name", "value": "rasa"}
        ],
...
async def test_interactive_domain_persistence(mock_endpoint, tmpdir):
    ...
    with aioresponses() as mocked:
        ...
        await interactive._write_domain_to_file(
            domain_path, events, mock_endpoint
        )
