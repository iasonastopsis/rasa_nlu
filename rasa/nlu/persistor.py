...
class Persistor(object):
    ...
    def retrieve(
        self, model_name: Text, project: Text, target_path: Text
    ) -> None:
    ...
    def _decompress(compressed_path: Text, target_path: Text) -> None:
        ...
        with tarfile.open(compressed_path, "r:gz") as tar:
            tar.extractall(
                target_path
            )  # project dir will be created if it not exists
...
class AWSPersistor(Persistor):
    ...
    def __init__(
        self, bucket_name: Text, endpoint_url: Optional[Text] = None
    ) -> None:
        import boto3
...
class GCSPersistor(Persistor):
    ...
    def list_models(self, project: Text) -> List[Text]:
        ...
        try:
            ...
            return [
                self._project_and_model_from_filename(b.name)[1]
    ...
    def list_projects(self) -> List[Text]:
        ...
        try:
            blob_iterator = self.bucket.list_blobs()
            projects_set = {
                self._project_and_model_from_filename(b.name)[0]
                for b in blob_iterator
            }
            return list(projects_set)
        except Exception as e:
            logger.warning(
                "Failed to list projects in "
...
class AzurePersistor(Persistor):
    ...
    def _persist_tar(self, file_key: Text, tar_path: Text) -> None:
        ...
        self.blob_client.create_blob_from_path(
            self.container_name, file_key, tar_path
        )
