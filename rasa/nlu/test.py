...
def save_json(data, filename):
    ...
    utils.write_to_file(
        filename, json.dumps(data, indent=4, ensure_ascii=False)
    )
...
def collect_nlu_errors(intent_results, errors_filename):
    ...
    if errors:
        ...
        logger.info(
            "Model prediction errors saved to {}.".format(errors_filename)
        )
        logger.debug(
...
def plot_intent_confidences(intent_results, intent_hist_filename):
    ...
    plt.gcf().clear()
    pos_hist = [
        r.confidence for r in intent_results if r.target == r.prediction
    ]
...
def merge_labels(aligned_predictions, extractor=None):
    ...
    if extractor:
        label_lists = [
            ap["extractor_labels"][extractor] for ap in aligned_predictions
        ]
    else:
...
def do_entities_overlap(entities):
    ...
    for i in range(len(sorted_entities) - 1):
        ...
        if (
            ...
            logger.warn(
                "Overlapping entity {} with {}".format(curr_ent, next_ent)
            )
            return True
...
def pick_best_entity_fit(token, candidates):
    ...
    if len(candidates) == 0:
        ...
        best_fit = np.argmax(
            [determine_intersection(token, c) for c in candidates]
        )
...
def determine_token_labels(token, entities, extractors):
    ...
    if len(entities) == 0:
        return "O"
    if not do_extractors_support_overlap(extractors) and do_entities_overlap(
        entities
    ):
...
def align_all_entity_predictions(targets, predictions, tokens, extractors):
    ...
    for ts, ps, tks in zip(targets, predictions, tokens):
        aligned_predictions.append(
            align_entity_predictions(ts, ps, tks, extractors)
        )
...
def get_predictions(interpreter, test_data, intent_targets):  # pragma: no cover
    ...
    for e, target in tqdm(
        samples_with_targets, total=len(test_data.training_examples)
    ):
...
def get_entity_extractors(interpreter):
    ...
    extractors = set(
        [c.name for c in interpreter.pipeline if "entities" in c.provides]
    )
...
def combine_result(intent_results, entity_results, interpreter, data):
    ...
    intent_current_result, entity_current_result = compute_metrics(
        interpreter, data
    )
...
def cross_validate(
    ...
    return (
        CVEvaluationResult(
            dict(intent_train_results), dict(intent_test_results)
        ),
        CVEvaluationResult(
            dict(entity_train_results), dict(entity_test_results)
        ),
    )
...
def compute_metrics(interpreter, corpus):
    ...
    intent_metrics = _compute_intent_metrics(
        intent_results, interpreter, corpus
    )
...
def return_results(results, dataset_name):
    ...
    for k, v in results.items():
        logger.info(
            "{} {}: {:.3f} ({:.3f})".format(
                dataset_name, k, np.mean(v), np.std(v)
            )
        )
