...
class NGramFeaturizer(Featurizer):
    ...
    def train(
        ...
        **kwargs: Any
        ...
        logger.debug(
            "Ngram collection took {} seconds" "".format(time.time() - start)
        )

        for example in training_data.training_examples:
            updated = self._text_features_with_ngrams(
                example, self.best_num_ngrams
            )
    ...
    def _remove_in_vocab_words_from_sentence(self, example):
        ...
        non_words = "".join(
            [letter for letter in non_words if not letter.isdigit()]
        )
        non_words = " ".join(
            [word for word in non_words.split(" ") if word != ""]
        )
