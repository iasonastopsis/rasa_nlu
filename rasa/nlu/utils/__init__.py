...
def list_subdirectories(path: Text) -> List[Text]:
    ...
    return [
        fn for fn in glob.glob(os.path.join(path, "*")) if os.path.isdir(fn)
    ]
...
def is_model_dir(model_dir: Text) -> bool:
    ...
    only_valid_files = all(
        [ext in allowed_extensions for ext in file_extenstions]
    )
...
def create_temporary_file(
    data: Any, suffix: Text = "", mode: Text = "w+"
) -> Text:
    ...
    def request(
        ...
        **kwargs: Any
        ...
        return requests.request(
            method, url, headers=headers, params=params, auth=auth, **kwargs
        )
