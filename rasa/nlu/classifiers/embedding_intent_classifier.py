...
class EmbeddingIntentClassifier(Component):
    ...
    def _create_intent_dict(training_data: "TrainingData") -> Dict[Text, int]:
        ...
        return {
            intent: idx for idx, intent in enumerate(sorted(distinct_intents))
        }
    ...
    def _create_encoded_intents(
        self, intent_dict: Dict[Text, int]
    ) -> np.ndarray:
        ...
        if self.intent_tokenization_flag:
            ...
            encoded_all_intents = np.zeros(
                (len(intent_dict), len(intent_token_dict))
            )
    ...
    def _prepare_data_for_training(
        ...
        X = np.stack(
            [e.get("text_features") for e in training_data.intent_examples]
        )

        intents_for_X = np.array(
            [
                intent_dict[e.get("intent")]
        ...
        Y = np.stack(
            [
                self.encoded_all_intents[intent_idx]
        ...
        **kwargs: Any
        ...
        with self.graph.as_default():
            ...
            self.a_in = tf.placeholder(
                tf.float32, (None, X.shape[-1]), name="a"
            )
            self.b_in = tf.placeholder(
                tf.float32, (None, None, Y.shape[-1]), name="b"
            )
            ...
            self.sim_op, sim_emb = self._tf_sim(
                self.word_embed, self.intent_embed
            )
    ...
    def process(self, message: "Message", **kwargs: Any) -> None:
        ...
        if self.session is None:
            ...
            if X.any() and intent_ids.size > 0:
                ...
                intent_ranking = [
                    {
                        "name": self.inv_intent_dict[intent_idx],
                        "confidence": score,
                    }
    ...
    def load(
        ...
        **kwargs: Any
        ...
        if model_dir and meta.get("file"):
            ...
            with io.open(
                os.path.join(model_dir, file_name + "_inv_intent_dict.pkl"),
                "rb",
            ) as f:
