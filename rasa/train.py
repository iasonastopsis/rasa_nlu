...
async def train_async(
    ...
    if not force_training and old_model:
        ...
        if not model.core_fingerprint_changed(
    ...
    if force_training or retrain_core:
        await train_core_async(
            domain, config, story_directory, output, train_path
        )
    else:
        print(
    ...
    if force_training or retrain_nlu:
        ...
        print(
            "NLU data / configuration did not change. "
...
async def train_core_async(
    ...
    if not train_path:
        ...
        new_fingerprint = model.model_fingerprint(
            config, domain, stories=stories
        )
        model.create_package_rasa(train_path, output_path, new_fingerprint)
        print_success(
            "Your Rasa Core model is trained and saved at '{}'."
            "".format(output_path)
        )
...
def train_nlu(
    ...
    if not train_path:
        ...
        print_success(
            "Your Rasa NLU model is trained and saved at '{}'."
            "".format(output_path)
        )
