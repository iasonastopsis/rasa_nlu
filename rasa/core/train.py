...
def create_argument_parser():
    ...
    subparsers = parser.add_subparsers(
        help="Training mode of core.", dest="mode"
    )
    ...
    for r in range(runs):
        ...
        for i in exclusion_percentages:
            ...
            for policy_config in policy_configs:
                ...
                if len(policies) > 1:
                    raise ValueError(
                        "You can only specify one policy per "
...
async def do_compare_training(cmdline_args, stories, additional_arguments):
    ...
    no_stories = await get_no_of_stories(
        cmdline_args.stories, cmdline_args.domain
    )
...
def main():
    ...
    if cmdline_arguments.mode == "default":
        loop.run_until_complete(
            do_default_training(
                cmdline_arguments, training_stories, additional_args
            )
        )

    elif cmdline_arguments.mode == "interactive":
        do_interactive_learning(
            cmdline_arguments, training_stories, additional_args
        )
