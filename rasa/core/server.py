...
def request_parameters(request):
    if request.method == "GET":
        ...
        try:
            ...
            logger.error(
                "Failed to decode json during respond request. "
                "Error: {}.".format(e)
            )
            raise
    ...
    def decorator(
        ...
        async def decorated(request: Request, *args: Any, **kwargs: Any) -> Any:
            ...
            if token is not None and provided == token:
                ...
                "USE_JWT"
            ) and request.app.auth.is_authenticated(request):
...
def create_app(
    ...
    async def execute_action(request: Request, sender_id: Text):
        ...
        verbosity = event_verbosity_parameter(
            request, EventVerbosity.AFTER_RESTART
        )
    ...
    async def retrieve_tracker(request: Request, sender_id: Text):
        ...
        if "ignore_restarts" in request.raw_args:
            ignore_restarts = utils.bool_arg(
                request, "ignore_restarts", default=False
            )
            if ignore_restarts:
    ...
    async def load_model(request: Request):
        ...
        domain_path = os.path.join(
            os.path.abspath(model_directory), "domain.yml"
        )
    ...
    async def evaluate_stories(request: Request):
        ...
        tmp_file = rasa.nlu.utils.create_temporary_file(
            request.body, mode="w+b"
        )
    ...
    async def continue_training(request: Request):
        ...
        try:
            # Fetches the appropriate bot response in a json format
            app.agent.continue_training(
                [tracker], epochs=epochs, batch_size=batch_size
            )
            return response.text("", 204)
