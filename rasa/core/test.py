...
def create_argument_parser():
    ...
    cli.arguments.add_model_and_story_group(
        parent_parser, allow_pretrained_model=False
    )
...
class EvaluationStore(object):
    ...
    def serialise_predictions(
        ...
        return [
            json.dumps(t) if isinstance(t, dict) else t for t in predictions
        ]
...
async def _generate_trackers(
    resource_name, agent, max_stories=None, use_e2e=False
):
...
def _collect_user_uttered_predictions(
    ...
    if entity_gold or predicted_entities:
        if len(entity_gold) > len(predicted_entities):
            ...
            entity_gold = pad_list_to_size(
                entity_gold, len(predicted_entities), "None"
            )
...
async def test(
    ...
    completed_trackers = await _generate_trackers(
        stories, agent, max_stories, use_e2e
    )
...
def log_evaluation_table(
    ...
    logger.info(
        "\tIn-data fraction: {:.3g}" "".format(in_training_data_fraction)
    )
...
def plot_story_evaluation(
    ...
    fig.savefig(
        os.path.join(out_directory, "story_confmat.pdf"), bbox_inches="tight"
    )
...
async def compare(models: Text, stories_file: Text, output: Text) -> None:
    ...
    for run in nlu_utils.list_subdirectories(models):
        ...
        for model in sorted(nlu_utils.list_subdirectories(run)):
            ...
            num_correct_run[policy_name].append(
                no_of_stories - len(failed_stories)
            )

        for k, v in num_correct_run.items():
            num_correct[k].append(v)

    utils.dump_obj_as_json_to_file(
        os.path.join(output, "results.json"), num_correct
    )
...
def plot_curve(output: Text, no_stories: List[int]) -> None:
    ...
    plt.savefig(
        os.path.join(output, "model_comparison_graph.pdf"), format="pdf"
    )
    plt.show()
...
def main():
    ...
    if not cmdline_arguments.core:
        raise ValueError(
            "you must provide a core model directory to evaluate "
            "using -d / --core"
        )
