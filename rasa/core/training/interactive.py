...
async def send_action(
    ...
    try:
        return await endpoint.request(
            json=payload, method="post", subpath=subpath
        )
    except ClientError:
        if is_new_action:
...
def _selection_choices_from_intent_prediction(
    ...
    sorted_intents = sorted(
        predictions, key=lambda k: (-k["confidence"], k["name"])
    )
...
def _slot_history(tracker_dump: Dict[Text, Any]) -> List[Text]:
    ...
    for k, s in tracker_dump.get("slots").items():
        colored_value = cliutils.wrap_with_color(
            str(s), rasa.cli.utils.bcolors.WARNING
        )
...
async def _request_action_from_user(
    ...
    question = questionary.select(
        "What is the next action of the bot?", choices
    )
...
def _collect_messages(evts: List[Dict[Text, Any]]) -> List[Message]:
    ...
    for evt in evts:
        if evt.get("event") == UserUttered.type_name:
            ...
            msg = Message.build(
                data["text"], data["intent"]["name"], data["entities"]
            )
            msgs.append(msg)
...
async def _write_nlu_to_file(
    ...
    try:
        ...
        logger.exception(
            "An exception occurred while trying to load the " "NLU data."
        )
...
def _entities_from_messages(messages):
    ...
    return list(
        {e["entity"] for m in messages for e in m.data.get("entities", [])}
    )
...
async def _correct_wrong_action(
    ...
    if finetune:
        await send_finetune(
            endpoint, result.get("tracker", {}).get("events", [])
        )
...
async def _confirm_form_validation(action_name, tracker, endpoint, sender_id):
    ...
    validate_input = await _ask_questions(
        validation_questions, sender_id, endpoint
    )
...
async def _validate_action(
    ...
    if _form_is_rejected(action_name, tracker):
        ...
        await _confirm_form_validation(
            action_name, tracker, endpoint, sender_id
        )
...
def _validate_user_regex(
    latest_message: Dict[Text, Any], intents: List[Text]
) -> bool:
...
async def _validate_user_text(
    ...
    if intent is None:
        print(
            "The NLU classification for '{}' returned '{}'"
            "".format(text, intent)
        )
        return False
    else:
...
async def _enter_user_message(
    ...
    message = await _ask_questions(
        question, sender_id, endpoint, lambda a: not a
    )
...
async def is_listening_for_message(
    ...
    tracker = await retrieve_tracker(
        endpoint, sender_id, EventVerbosity.APPLIED
    )
...
def _print_help(skip_visualization: bool) -> None:
    ...
    if not skip_visualization:
        ...
        visualization_help = (
            "Visualisation at {}/visualization.html."
