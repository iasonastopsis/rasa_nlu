...
class StoryStepBuilder(object):
    ...
    def _prev_end_checkpoints(self):
        if not self.current_steps:
            ...
            end_names = {
                e.name for s in self.current_steps for e in s.end_checkpoints
            }
    ...
    def ensure_current_steps(self):
        completed = [
            step for step in self.current_steps if step.end_checkpoints
        ]
        unfinished = [
            step for step in self.current_steps if not step.end_checkpoints
        ]
    ...
    async def read_from_file(
        ...
        try:
            ...
            reader = StoryFileReader(
                domain, interpreter, template_variables, use_e2e
            )
            return await reader.process_lines(lines)
        except ValueError as err:
            file_info = (
                "Invalid story file format. Failed to parse "
        ...
        for idx, line in enumerate(lines):
            ...
            try:
                ...
                if line.strip() == "":
                    ...
                    user_messages = [
                        el.strip() for el in line[1:].split(" OR ")
                    ]
                    if self.use_e2e:
