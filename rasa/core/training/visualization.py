...
def _incoming_edges(graph, node):
    return {
        (prev_node, k) for prev_node, _, k in graph.in_edges(node, keys=True)
    }


def _outgoing_edges(graph, node):
    return {
        (succ_node, k) for _, succ_node, k in graph.out_edges(node, keys=True)
    }
...
def _length_of_common_action_prefix(
    this: List[Event], other: List[Event]
) -> int:
...
def _add_default_nodes(
    graph: "networkx.MultiDiGraph", fontsize: int = 12
) -> None:
    ...
    graph.add_node(
        TMP_NODE_ID, label="TMP", style="invis", **{"class": "invisible"}
    )
...
async def visualize_neighborhood(
    ...
    for events in event_sequences:
        ...
        if is_current:
            ...
            graph.add_node(
                special_node_idx, label="...", **{"class": "ellipsis"}
            )
