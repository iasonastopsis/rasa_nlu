...
TrackersTuple = Tuple[
    List[TrackerWithCachedStates], List[TrackerWithCachedStates]
]
    ...
    def generate(self, silent: bool = False) -> List[TrackerWithCachedStates]:
        ...
        while not everything_reachable_is_reached or phase < min_num_aug_phases:
            phase_name = self._phase_name(
                everything_reachable_is_reached, phase
            )
            ...
            if num_active_trackers:
                ...
                logger.debug(
                    "There are no trackers for {}" "".format(phase_name)
                )
                break
            ...
            logger.debug(
                "Finished phase ({} training samples found)."
                "".format(num_finished)
            )
        ...
        logger.debug(
            "Found {} training trackers." "".format(len(finished_trackers))
        )
    ...
    def _filter_active_trackers(
        ...
        for start_name in unused_checkpoints:
            ...
            if start_name != STORY_START:
                ...
                next_active_trackers[start_name] = active_trackers.get(
                    start_name, []
                )
