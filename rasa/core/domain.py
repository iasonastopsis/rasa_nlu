...
class Domain(object):
    ...
    def from_dict(cls, data):
        ...
        intent_properties = cls.collect_intent_properties(
            data.get("intents", {})
        )
        return cls(
            intent_properties,
    ...
    def add_requested_slot(self):
        if self.form_names and REQUESTED_SLOT not in [
    ...
    def actions(self, action_endpoint):
        return [
            self.action_for_name(name, action_endpoint)
    ...
    def _raise_action_not_found_exception(self, action_name):
        action_names = "\n".join(
            ["\t - {}".format(a) for a in self.action_names]
        )
        raise NameError(
