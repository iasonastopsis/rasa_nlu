...
class ReminderCancelled(Event):
    ...
    def _from_story_string(
        ...
        return [
            ReminderCancelled(
                parameters.get("action"), parameters.get("timestamp")
            )
        ]
...
class FollowupAction(Event):
    ...
    def _from_story_string(
        ...
        return [
            FollowupAction(parameters.get("name"), parameters.get("timestamp"))
        ]
...
class ActionExecuted(Event):
    ...
    def __init__(
        self, action_name, policy=None, confidence=None, timestamp=None
    ):
        self.action_name = action_name
        self.policy = policy
    ...
    def __str__(self):
        return (
            "ActionExecuted(action: {}, policy: {}, confidence: {})"
...
class AgentUttered(Event):
    ...
    def _from_parameters(cls, parameters):
        try:
            ...
            raise ValueError(
                "Failed to parse agent uttered event. " "{}".format(e)
            )
...
class FormValidation(Event):
    ...
    def _from_parameters(cls, parameters):
        return FormValidation(
            parameters.get("validate"), parameters.get("timestamp")
        )
