...
async def load_from_server(
    ...
    if wait_time_between_pulls:
        ...
        await schedule_model_pulling(
            model_server, int(wait_time_between_pulls), agent
        )
...
async def _update_model_from_server(
    ...
    if new_model_fingerprint:
        _load_and_set_updated_model(
            agent, model_directory, new_model_fingerprint
        )
    else:
...
async def _pull_model_and_fingerprint(
    ...
    logger.debug(
        "Requesting model from server {}..." "".format(model_server.url)
    )
...
class Agent(object):
    ...
    async def handle_message(
        ...
        **kwargs
        ...
        if not lock:
            logger.debug(
                "created a new lock for conversation '{}'"
    ...
    def continue_training(
        ...
        if not self.is_ready():
            raise AgentNotReady(
                "Can't continue training without a policy " "ensemble."
            )
    ...
    def _are_all_featurizers_using_a_max_history(self):
        ...
        def has_max_history_featurizer(policy):
            return policy.featurizer and hasattr(
                policy.featurizer, "max_history"
            )
    ...
    def handle_channels(
        ...
        app = run.configure_app(
            channels, cors, None, enable_api=False, route=route
        )
    ...
    def persist(
        self, model_path: Text, dump_flattened_stories: bool = False
    ) -> None:
        ...
        logger.info(
            "Persisted model to '{}'" "".format(os.path.abspath(model_path))
        )
