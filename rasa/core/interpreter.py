...
class NaturalLanguageInterpreter(object):
    ...
    def create(obj, endpoint=None):
        ...
        if len(name_parts) == 1:
            ...
            return RasaNLUHttpInterpreter(
                name_parts[1], endpoint, name_parts[0]
            )
        else:
            raise Exception(
    ...
    def _parse_parameters(
        ...
        try:
            ...
            if isinstance(parsed_entities, dict):
                return RegexInterpreter._create_entities(
                    parsed_entities, sidx, eidx
                )
            else:
                raise Exception(
