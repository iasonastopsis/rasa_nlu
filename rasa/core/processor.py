...
class MessageProcessor(object):
    ...
    async def _predict_and_execute_next_action(self, message, tracker):
        ...
        dispatcher = Dispatcher(
            message.sender_id, message.output_channel, self.nlg
        )
    ...
    async def _run_action(
        ...
        try:
            ...
            events = [
                ActionExecutionRejected(action.name(), policy, confidence)
            ]
        ...
        self._log_action_on_tracker(
            tracker, action.name(), events, policy, confidence
        )
