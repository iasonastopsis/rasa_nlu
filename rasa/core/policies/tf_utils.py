    ...
    def __call__(self, attn_inputs, scores, scores_state, mask):
        ...
        if mask is not None:
            ...
            if self._shift_weight is not None:
                ...
                scores = tf.where(
                    mask > 0, scores, -self._inf * tf.ones_like(scores)
                )
...
def _compute_time_attention(
    ...
    timed_time_mask = tf.concat(
        [timed_time_mask, tf.ones_like(time_mask[:, :1])], 1
    )
    ...
    next_attention_state = tf.concat(
        [next_scores_state, zeros[:, time + 1 :]], 1
    )
...
class TimeAttentionWrapper(tf.contrib.seq2seq.AttentionWrapper):
    ...
    def __init__(
        ...
        self._timed_ntms = [
            TimedNTM(attn_shift_range[0], sparse_attention, name="0")
        ]
        if self._is_multi:
    ...
    def zero_state(self, batch_size, dtype):
        ...
        # use AttentionWrapperState from superclass
        zero_state = super(TimeAttentionWrapper, self).zero_state(
            batch_size, dtype
        )
        ...
        with tf.name_scope(
            ...
            all_time_masks = tf.TensorArray(
                ...
                0,
                tf.zeros(
                    [batch_size, self.state_size.all_time_masks], tf.int32
                ),
            )
    ...
    def call(self, inputs, state):
        ...
        rnn_inputs, attn_inputs = self._rnn_and_attn_inputs_fn(
            inputs, cell_state
        )
    ...
    def _read_from_tensor_array(tensor_array, time):
        ...
        return tf.transpose(
            tensor_array.gather(tf.range(0, time + 1)), [1, 0, 2]
        )
    ...
    def _apply_alignments_to_history(alignments, history_states, state):
        ...
        history_states = tf.concat(
            [history_states, tf.expand_dims(state, 1)], 1
        )
    ...
    def _additional_likelihood(self, output, prev_output, current_time_prob):
        ...
        return tf.where(
            current_time_prob < 0.5, likelihood, tf.ones_like(likelihood)
        )

    def _new_hidden_state(
        self, prev_all_cell_states, new_state, alignments, time
    ):
        ...
        prev_cell_states = self._read_from_tensor_array(
            prev_all_cell_states, time
        )[:, :-1, :]
...
class ChronoBiasLayerNormBasicLSTMCell(tf.contrib.rnn.LayerNormBasicLSTMCell):
    ...
    def _dense_layer(args, layer_size):
        ...
        weights = tf.get_variable(
            "kernel", [proj_size, layer_size], dtype=dtype
        )
