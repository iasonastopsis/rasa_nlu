...
class EmbeddingPolicy(Policy):
    ...
    def _create_y_for_action_listen(self, domain: "Domain") -> np.ndarray:
        ...
        return self.encoded_all_actions[
            action_listen_idx : action_listen_idx + 1
        ]
    ...
    def _create_tf_session_data(
        ...
        X, slots, previous_actions = self._create_X_slots_previous_actions(
            data_X
        )
    ...
    def _create_tf_no_intent_embed(
        self, x_for_no_intent_i: tf.Tensor
    ) -> tf.Tensor:
        ...
        return tf.stop_gradient(
            self._create_embed(
                x_for_no_intent, layer_name_suffix=layer_name_suffix
            )
        )

    def _create_tf_no_action_embed(
        self, y_for_no_action_in: tf.Tensor
    ) -> tf.Tensor:
        ...
        return tf.stop_gradient(
            self._create_embed(
                y_for_no_action, layer_name_suffix=layer_name_suffix
            )
        )
    ...
    def rnn_and_attn_inputs_fn(
        ...
        if isinstance(cell_state, tf.contrib.rnn.LSTMStateTuple):
            attn_inputs = tf.concat(
                [inputs[:, : self.embed_dim], cell_state.h], -1
            )
        else:
            attn_inputs = tf.concat(
                [inputs[:, : self.embed_dim], cell_state], -1
            )
    ...
    def _create_attn_cell(
        ...
        if self.attn_before_rnn:
            ...
            ignore_mask = tf.reduce_all(
                tf.equal(tf.expand_dims(embed_for_no_intent, 0), embed_utter),
                -1,
            )
        ...
        if self.attn_after_rnn:
            ...
            attn_mech_after_rnn = self._create_attn_mech(
                embed_prev_action, real_length
            )
        ...
        return TimeAttentionWrapper(
            ...
            cell_input_fn=lambda inputs, attention: (
                self.cell_input_fn(
                    inputs, attention, num_memory_units_before_rnn
                )
            ),
    ...
    def _create_tf_dial_embed(
        ...
        cell_input = tf.concat(
            [embed_utter, embed_slots, embed_prev_action], -1
        )
    ...
    def _all_time_masks_from(
        ...
        return tf.transpose(final_state.all_time_masks.stack(), [1, 0, 2])[
            :, :-1, :
        ]
    ...
    def _tf_sim(
        ...
        if self.similarity_type in {"cosine", "inner"}:
            ...
            if len(embed_dialogue.shape) == len(embed_action.shape):
                ...
                # two embedding vectors of the same size
                sim = tf.reduce_sum(
                    embed_dialogue * embed_action, -1, keepdims=True
                )
                bin_sim = tf.where(
    ...
    def train(
        ...
        **kwargs: Any
        ...
        with self.graph.as_default():
            ...
            self.slot_embed = self._create_embed(
                self.c_in, layer_name_suffix="slt"
            )
            ...
            # process rnn output
            if self.is_using_attention():
                self.alignment_history = self._alignments_history_from(
                    final_state
                )
            ...
            self.sim_op, sim_act = self._tf_sim(
                self.dial_embed, self.bot_embed, mask
            )
            # construct loss
    ...
    def _scale_loss_by_count_actions(
        ...
        if self.scale_loss_by_action_counts:
            full_X = np.concatenate(
                [X, slots, previous_actions, actions_for_Y[:, :, np.newaxis]],
                -1,
            )
            full_X = full_X.reshape((-1, full_X.shape[-1]))

            _, i, c = np.unique(
                full_X, return_inverse=True, return_counts=True, axis=0
            )
    ...
    def _calc_train_acc(
        self, session_data: SessionData, mask: tf.Tensor
    ) -> np.float32:
    ...
    def persist(self, path: Text) -> None:
        ...
        with self.graph.as_default():
            ...
            self._persist_tensor(
                "y_for_action_listen", self._y_for_action_listen_in
            )
