...
class Policy(object):
    ...
    def _get_valid_params(func: Callable, **kwargs: Any) -> Dict:
        ...
        logger.debug(
            "Parameters ignored by `model.fit(...)`: {}"
    ...
    def _training_data_for_continue_training(
        ...
        trackers = [
            training_trackers[i] for i in sampled_idx
        ] + training_trackers[-1:]
    ...
    def predict_action_probabilities(
        ...
        raise NotImplementedError(
            "Policy must have the capacity " "to predict."
        )

    def persist(self, path: Text) -> None:
        """Persists the policy to a storage."""
        raise NotImplementedError(
            "Policy must have the capacity " "to persist itself."
        )
    ...
    def load(cls, path: Text) -> "Policy":
        ...
        raise NotImplementedError(
            "Policy must have the capacity " "to load itself."
        )
