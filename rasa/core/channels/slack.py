...
class SlackBot(SlackClient, OutputChannel):
    ...
    def __init__(
        self, token: Text, slack_channel: Optional[Text] = None
    ) -> None:
...
class SlackInput(InputChannel):
    ...
    def from_credentials(cls, credentials):
        ...
        return cls(
            credentials.get("slack_token"), credentials.get("slack_channel")
        )
    ...
    def _is_button_reply(slack_event):
        ...
        return SlackInput._is_interactive_message(
            payload
        ) and SlackInput._is_button(payload)
    ...
    async def process_message(
        self, request: Request, on_new_message, text, sender_id
    ):
