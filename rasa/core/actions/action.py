...
def actions_from_names(
    ...
    return [
        action_from_name(name, action_endpoint, user_actions)
...
class ActionRestart(Action):
    ...
    async def run(self, dispatcher, tracker, domain):
        ...
        # only utter the template if it is available
        await dispatcher.utter_template(
...
class ActionDefaultFallback(Action):
    ...
    async def run(self, dispatcher, tracker, domain):
        ...
        await dispatcher.utter_template(
            "utter_default", tracker, silent_fail=True
        )
...
class RemoteAction(Action):
    ...
    async def run(self, dispatcher, tracker, domain):
        ...
        try:
            logger.debug(
                "Calling action endpoint to run action '{}'."
                "".format(self.name())
            )
...
class ActionExecutionRejection(Exception):
    ...
    def __init__(self, action_name, message=None):
        ...
        self.message = (
            message
            or "Custom action '{}' rejected to run" "".format(action_name)
        )
